...
- 배경: **항상 삼성전자 로고·브랜드가 드러나는 이미지로 고정**해요(`SAMSUNG` 로고·간판·매장 등 브랜드가 또렷이 보이는 컷). 특정 갤럭시 모델 사진(S24·S25 등)은 **금세 구형이 되므로 쓰지 말고**, 시대를 타지 않는 삼성 브랜드 이미지를 매주 재사용해요. 투명도는 약 **25~40%**로 낮춰 텍스트 가독성을 확보해요.
  - 현재 고정 asset: `MAHL9VlyeJA` (SAMSUNG 로고 도시 빌딩 · Pexels ID 28536869).
  - ❌ 특정 갤럭시 모델 단독 컷(구형화 우려), 아이폰·타사 기기, 삼성과 무관한 일반 테크·풍경 사진 금지.
